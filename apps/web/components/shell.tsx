      ...
      <header className="border-b border-slate-200 bg-white/90 backdrop-blur">
        <div className="mx-auto flex max-w-7xl flex-col gap-4 px-4 py-4 sm:flex-row sm:items-center sm:justify-between sm:px-6">
          <div>
            ...
            <h1 className="text-base font-semibold text-ink sm:text-lg">{translate("merchantOperatingSystem")}</h1>
          </div>
          <div className="flex w-full items-center justify-between gap-3 sm:w-auto sm:justify-end">
            <LanguageSwitcher />
            <button className="shrink-0 rounded-full bg-ink px-4 py-2 text-sm font-medium text-white" onClick={handleLogout}>
              {translate("logout")}
            </button>
          </div>
        </div>
            ...
            <nav className="grid grid-cols-2 gap-2 lg:block lg:space-y-2">
              {links.map((link) => (
                <Link
                  key={link.href}
                  href={link.href}
                  className={`block rounded-2xl px-4 py-3 text-center text-sm font-medium ${pathname === link.href ? "bg-brand-600 text-white" : "text-slate-600 hover:bg-slate-50"}`}
                >
                  {link.label}
                </Link>
              ))}
